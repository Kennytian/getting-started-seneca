module.exports = function (options) {
    ...
    this.add('role:store, get:book', function (msg, reply) {
        this.make('book').load$(msg.id, reply);
    });

    // 从数据库中，查询一本ID为 `msg.id` 的书，我们使用了 `load$` 方法
    this.add('role:store, add:book', function (msg, reply) {
        this.make('book').data$(msg.data).save$(reply);
    });
    ...
    // 需要它。
    this.add('role:store, cmd:purchase', function (msg, reply) {
        this.make('book').load$(msg.id, function (err, book) {
            ...
            this.make('purchase').data$({
                when: Date.now(), bookId: book.id, title: book.title, price: book.price
    ...
    // `fatal` 方法用于打印相应级别的日志。
    this.add('role:store,info:purchase', function (msg, reply){
